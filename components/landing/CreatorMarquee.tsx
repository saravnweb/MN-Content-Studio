...
const ROW_1 = [
  { name: 'Arjun Singh',    niche: 'Tech',      platform: 'YouTube',   followers: '125K', rating: 4.8, initial: 'A', bg: 'bg-indigo-600' },
  { name: 'Meera Reddy',    niche: 'Fitness',   platform: 'Instagram', followers: '89K',  rating: 4.9, initial: 'M', bg: 'bg-emerald-600' },
  { name: 'Priya Kapoor',   niche: 'Food',      platform: 'Instagram', followers: '210K', rating: 4.7, initial: 'P', bg: 'bg-amber-600' },
  { name: 'Rahul Mehta',    niche: 'Finance',   platform: 'YouTube',   followers: '180K', rating: 4.8, initial: 'R', bg: 'bg-violet-600' },
  { name: 'Ananya Tiwari',  niche: 'Beauty',    platform: 'Instagram', followers: '95K',  rating: 4.9, initial: 'A', bg: 'bg-rose-600' },
  { name: 'Vikram Nair',    niche: 'Travel',    platform: 'YouTube',   followers: '150K', rating: 4.6, initial: 'V', bg: 'bg-sky-600' },
]

const ROW_2 = [
  { name: 'Sneha Banerjee', niche: 'Lifestyle', platform: 'Instagram', followers: '67K',  rating: 4.7, initial: 'S', bg: 'bg-orange-600' },
  { name: 'Karan Desai',    niche: 'Gaming',    platform: 'YouTube',   followers: '320K', rating: 4.8, initial: 'K', bg: 'bg-purple-600' },
  { name: 'Divya Sharma',   niche: 'Fashion',   platform: 'Instagram', followers: '145K', rating: 4.9, initial: 'D', bg: 'bg-pink-600' },
  { name: 'Akash Patel',    niche: 'Health',    platform: 'YouTube',   followers: '98K',  rating: 4.7, initial: 'A', bg: 'bg-teal-600' },
  { name: 'Nisha Gupta',    niche: 'Education', platform: 'YouTube',   followers: '230K', rating: 4.8, initial: 'N', bg: 'bg-cyan-600' },
  { name: 'Rohit Verma',    niche: 'Sports',    platform: 'Instagram', followers: '175K', rating: 4.6, initial: 'R', bg: 'bg-lime-700' },
]
...
function CreatorCard({ name, niche, platform, followers, rating, initial, bg }: typeof ROW_1[0]) {
  ...
  return (
    <div className="w-56 shrink-0 bg-gray-900 border border-gray-800 rounded-2xl p-4 mx-2">
      <div className="flex items-center gap-3 mb-3">
        <div className={`w-10 h-10 rounded-full ${bg} flex items-center justify-center text-sm font-bold text-white shrink-0`}>
          {initial}
        </div>
      ...
      <div className="flex items-center gap-2">
        <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${nicheStyle}`}>{niche}</span>
        <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${platformStyle}`}>{platform}</span>
        <span className="ml-auto text-[10px] text-yellow-400 font-medium">★ {rating}</span>
      </div>
    </div>
  )
}
